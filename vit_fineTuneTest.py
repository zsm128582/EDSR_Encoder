...
from models.model_vit import VisionTransformer 
...
def prepare_training():
    ...
    if (config.get('resume') is not None) and os.path.exists(config.get('resume')):
        ...
        model = VisionTransformer(img_size  =256 ,patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6),num_classes = 1000 , drop_path_rate = 0.1, global_pool = True)
        ...
        sv_file = torch.load(config['finetune'],map_location=torch.device('cpu'))
        model = VisionTransformer(img_size  =256,patch_size=16, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
            norm_layer=partial(nn.LayerNorm, eps=1e-6),num_classes = 1000 , drop_path_rate = 0.1, global_pool = True)
    
        model = model.cuda()
        checkpoint_model = sv_file['model']
